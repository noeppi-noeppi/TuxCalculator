...
\begin{document}
    ...
    An error value can be created by placing arbitrary text between double quotation marks.
    ...
    \end{code}
    The first variant sets a variable to the last answer, the second variant sets a variable to the provided value.
    ...
    \subsection{\texttt{cat}}\label{subsec:cmd-cat}
    The \verb|cat| command can manipulate catcodes.
    ...
    TuxCalculator can also assign a catcode to a sequence of characters.
    When reading input, always the longest matching catcode is used.
    ...
    Changing catcodes in accomplished by the \verb|cat| command.
    ...
    While each single character always has a catcode (if nothing was set, its \verb|invalid|), only specific longer sequences of characters have a dedicated catcode.
    The catcode of a longer sequence of characters can be unset by assigning it the \verb|invalid| catcode.
    ...
    \begin{itemize}
        \item If the error token appears in a \verb|cat| or \verb|dump| command, no interpolation is performed.
        \item Whenever a token with the \verb|interpolate| catcode is found that is not escaped by a backslash and immediately followed by a token with \verb|letter| or \verb|exp| catcode, the longest sequence of tokens with \verb|letter|, \verb|digit| or \verb|exp| catcodes is used as the name for a variable to interpolate.
              If instead the \verb|interpolate| token is followed by a token with the \verb|escape| catcode, an escaped identifier is read using the standard escaping rules with the exception that every error token between the delimiting escape tokens must be escaped with a backslash.
    ...
    \begin{center}
        \begin{tabular}{p{0.3\textwidth}p{0.5\textwidth}}
            ...
            \verb|error|   & The double quotation mark \verb|"|. \\
    ...
    \begin{longtable}{p{0.2\textwidth}p{\dimexpr0.8\textwidth-3\tabcolsep\relax}}
        ...
        $ \mathrm{range}(a, b, s) $                & $ \mathrm{range}(a, b) $ with step-size $ s $. Negative step-sizes behave as their absolute value. Step size $ 0 $ gives the list containing just the bounds. \\
        ...
        $ \mathrm{chebyshev}(n) $                  & Computes the $ n $th chebyshev polynomial. \tabgap
    ...
    \begin{longtable}{p{0.2\textwidth}p{\dimexpr0.8\textwidth-3\tabcolsep\relax}}
        ...
        \verb|#y|         & Enables recursion by transforming a function $ f(s,a\dots) $ into a function $ g(a\dots) $ that calls $ f $ with the parameter $ s $ being equivalent to the function $ g $ itself. \\

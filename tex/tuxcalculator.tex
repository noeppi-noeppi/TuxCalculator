...
\begin{document}
    ...
    \paragraph{The swing frontend} can be set using \verb|--gui=swing| and is the default if TuxCalculator can't use \verb|gtk| and is not running in headless mode.
    ...
    \begin{longtable}{p{0.2\textwidth}p{\dimexpr0.8\textwidth-3\tabcolsep\relax}}
        ...
        $ \mathrm{abs}(x) $                        & Calculates the absolute value of $ x $ which for a number $ a + bi$ is $ \sqrt{a^2 + b^2} $. \\
        $ \mathrm{sgn}(x) $                        & Calculates the sign of $ x $ which is the point on the unit circle that holds $ \mathrm{abs}(x) \cdot \mathrm{sgn}(x) = x $. The sign of $ 0 $ is $ 0 $. \tabgap

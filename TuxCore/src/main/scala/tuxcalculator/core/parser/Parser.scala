...
class CalculatorParsers(val ctx: ParsingContext) extends Parsers  {
  ...
  def parseTokens[T](parser: Parser[T], tokens: TokenStream): ParseResult[T] = parser.apply(new TokenReader(tokens.tokens.toList)) match {
    case Success(_, remaining) if !remaining.atEnd => Error("Input not fully consumed", remaining)
    case result => result
  }
  
  def flatAcceptMatch[U](expected: String, f: PartialFunction[Elem, ParseResult[U]]): Parser[U] = Parser {
  ...
  def leftRec[A, B](init: Parser[A], follow: Parser[B], combine: (A, B) => A): Parser[A] = {
    init ~ rep(follow) ^^ {
      case head ~ elems => elems.foldLeft(head)(combine)
    }
  }
  
  def leftRec1N[A, B](init: Parser[A], immediateFollow: Parser[B], follow: Parser[B], combine: (A, B) => A): Parser[A] = {
  ...
  // Utilities
  def identifier: Parser[String] = acceptMatch("identifier", {
    case Token.Identifier(name) => name
  })
  
  private def parameter: Parser[(String, Boolean)] = identifier ~ opt(accept(Token.Vararg)) ^^ {
    case name ~ Some(_) => (name, true)
    case name ~ None => (name, false)
  }
  def signature: Parser[Ast.Signature] = { repsep(parameter, accept(Token.ElementSep)) ^^ (x => x.toVector) } ^? ({
  ...
  def guard: Parser[Option[Ast.Expression]] = opt(accept(Token.Guard) ~> expression)
  private def guarded_parameter: Parser[(String, Boolean, Option[Ast.Expression])] = parameter ~ guard ^^ {
    case (name, vararg) ~ guard => (name, vararg, guard)
  }
  def guarded_signature: Parser[(Ast.Signature, Vector[Option[Ast.Expression]])] = { repsep(guarded_parameter, accept(Token.ElementSep)) ^^ (x => x.toVector) } ^? ({
  ...
  private def target_func: Parser[Ast.DefTarget] = identifier ^^ (name => Ast.DefTarget.Function(name))
  private def target_other: Parser[Ast.DefTarget] = acceptMatch("def operator", {
  ...
  private def target: Parser[Ast.DefTarget] = target_func | target_other | failure("identifier or operator expected")
  ...
  private def match_entry: Parser[Ast.MatchEntry] = flatAcceptMatch("guarded argument list", {
  ...
  def variable: Parser[Ast.Variable] = identifier ^^ (name => Ast.Variable(name))
  def reference: Parser[Ast.Reference] = accept(Token.Reference) ~> target ^^ (name => Ast.Reference(name))
  def special: Parser[Ast.Special] = accept(Token.Special) ~> identifier ^^ (name => Ast.Special(name))
  ...
  def post_action_any_apply: Parser[PostOperation] = post_action_partial_apply_direct | post_action_partial_apply_simple | post_action_apply
  def applied_expression: Parser[Ast.Expression] = leftRec[Ast.Expression, PostOperation](basic_expression, post_action_any_apply, (base, apply) => (base, apply) match {
  ...
  def signed_expression: Parser[Ast.Expression] = explicitly_signed_expression | applied_expression
  def explicitly_signed_expression: Parser[Ast.Expression] = acceptMatch("sign", { case Token.Sign(name) => name }) ~ signed_expression ^^ {
    case sgn ~ expr => Ast.SignApplication(sgn, expr)
  }
  
  def post_action_op: Parser[PostOperation] = acceptMatch("postfix", { case Token.Post(name) => PostOperation.Operator(name) })
  def post_action: Parser[PostOperation] = post_action_any_apply | post_action_op
  def post_expression: Parser[Ast.Expression] = leftRec1N[Ast.Expression, PostOperation](signed_expression, post_action_op, post_action, (base, apply) => (base, apply) match {
    case (expr, PostOperation.Application(app)) => Ast.Application(expr, app)
    case (expr, PostOperation.PartialApplication(app)) => Ast.PartialApplication(expr, app)
  ...
  def shorthand_expression: Parser[Ast.Expression] = (in: Input) => {
  ...
  def operator_application: Parser[(String, Ast.Expression)] = acceptMatch("operator", {
    case Token.Operator(op) => op
  }) ~ shorthand_expression ^^ { case op ~ expr => (op, expr) }
  
  def operator_expression: Parser[Ast.Expression] = shorthand_expression ~ rep(operator_application) ^^ {
    case expr ~ Nil => expr
    case expr ~ ops => Ast.OperatorApplication(expr, ops.toVector)
  }
  
  def expression: Parser[Ast.Expression] = operator_expression
  def expression_list: Parser[List[Ast.Expression]] = repsep(expression, accept(Token.ElementSep))
  def expression_grouped_list: Parser[List[List[Ast.Expression]]] = repsep(expression_list, accept(Token.GroupSep))
  
  def argument: Parser[Ast.Argument] = expression ~ opt(accept(Token.Vararg)) ^^ {
  ...
  def cmd_set: Parser[Ast.SetCommand] = identifier ^^ (name => Ast.SetCommand(name))

  private def cmd_def_priority: Parser[Ast.Expression] = flatAcceptMatch("priority", {
    case Token.List(tokens) => parseTokens(expression, tokens)
  })
  def cmd_def: Parser[Ast.DefCommand] = opt(cmd_def_priority) ~ target ~ flatAcceptMatch("argument list", {
  ...
  def cmd_cat: Parser[Ast.CatCommand] = flatAcceptMatch("character", {
  ...
  def cmd_tok: Parser[Ast.TokCommand] = flatAcceptMatch("token", {
  ...
  def cmd_dump: Parser[Ast.DumpCommand] = flatAcceptMatch("file name", {
    case Token.Error(msg) => msg match {
